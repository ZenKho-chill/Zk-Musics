...
export class ButtonSkip {
  ...
  constructor(
    ...
    this.accessableby = client.config.SETUP_BUTTON.skip;
    this.execute();
  }
  ...
  async execute() {
    ...
    if (this.player.queue.size == 0 && this.player.data.get("autoplay") !== true) {
      const embed = new EmbedBuilder()
        .setDescription(
          `${this.client.i18n.get(this.language, "server.handlers", "skip_notfound")}`
        )
        .setColor(this.client.color_main);
      ...
      const embed = new EmbedBuilder()
        .setDescription(`${this.client.i18n.get(this.language, "server.handlers", "skip_msg")}`)
        .setColor(this.client.color_main);
