  ...
  async run(
    ...
    if (player.queue.size == 0 && player.data.get("autoplay") !== true)
      return new ReplyInteractionService(
        client,
        message,
        `${client.i18n.get(language, "client.ui", "skip_notfound")}`
      );
    ...
    new ReplyInteractionService(
      client,
      message,
      `${client.i18n.get(language, "client.ui", "skip_msg", {
        user: message.user?.id
          ? `<@${message.user.id}>`
          : `${message.user?.tag || client.i18n.get(language, "server.handlers.buttons", "user_unknown")}`,
        title: currentTrack ? this.getTitle(client, currentTrack) : client.i18n.get(language, "server.handlers.buttons", "title_unknown"),
      })}`
    );
  }
