...
export default class implements PlayerButton {
  name = "skip";
  accessableby = data.PLAYER_BUTTON.skip;
  async run(
    client: Manager,
    ...
    log.info("Skip button pressed", `User: ${message.user.id} | Guild: ${message.guild?.id} | Player: ${player.guildId}`);
    ...
    if (player.queue.size == 0 && player.data.get("autoplay") !== true)
      return new ReplyInteractionService(
        client,
        message,
        `${client.i18n.get(language, "button.player.music", "skip_notfound")}`
      );
    ...
    new ReplyInteractionService(
      client,
      message,
      `${client.i18n.get(language, "button.player.music", "skip_msg", {
        user: message.user?.id
          ? `<@${message.user.id}>`
          : `${message.user?.tag || "Người dùng không rõ"}`,
        title: currentTrack ? this.getTitle(client, currentTrack) : "Không xác định",
      })}`
    );
  }
