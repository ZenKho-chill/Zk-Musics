...
export default class implements PlayerButton {
  name = "volup";
  accessableby = data.PLAYER_BUTTON.volume_up;
  async run(
    client: Manager,
    ...
    log.info(message.user.id, "VolumeUp button pressed", message.guild?.id, `Player: ${player.guildId}`);
    ...
    const reply_msg = `${client.i18n.get(language, "button.player.music", "volup_msg", {
    ...
    if (player.volume >= 100) {
      new ReplyInteractionService(
        client,
        message,
        `${client.i18n.get(language, "button.player.music", "volume_max")}`
      );
      return;
    }
