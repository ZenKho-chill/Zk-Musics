...
export default class implements PlayerButton {
  ...
  async run(
    ...
    if (this.accessableby === "PremiumGuild" && !userPerm.GuildPremium) {
      const embed = new EmbedBuilder()
        .setAuthor({
          name: client.i18n.get(language, "server.handlers", "no_premium_guild_author"),
        })
        .setDescription(
          `${client.i18n.get(language, "server.handlers", "no_premium_guild_desc", {
            user: message.user?.id
              ? `<@${message.user.id}>`
              : `${message.user?.tag || "Người dùng không rõ"}`,
            serversupport: client.config.bot.SERVER_SUPPORT_URL,
      ...
      if (client.config.MENU_HELP_EMOJI.E_PREMIUM && client.config.bot.PREMIUM_URL) {
        PremiumCheckButton.addComponents(
          new ButtonBuilder()
            .setLabel(client.i18n.get(language, "server.handlers", "no_premium_guild_button"))
            .setStyle(ButtonStyle.Link)
            .setEmoji(client.config.MENU_HELP_EMOJI.E_PREMIUM)
            .setURL(client.config.bot.PREMIUM_URL)
        );
    ...
    const reply_msg = `${client.i18n.get(language, "client.ui", "voldown_msg", {
    ...
    if (player.volume <= 0.1) {
      new ReplyInteractionService(
        client,
        message,
        `${client.i18n.get(language, "client.ui", "volume_min")}`
      );
      return;
    }
