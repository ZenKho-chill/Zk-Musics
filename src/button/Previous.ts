...
export default class implements PlayerButton {
  name = "replay";
  accessableby = data.PLAYER_BUTTON.previous;
  async run(
    client: Manager,
    ...
    log.info(message.user.id, "Previous button pressed", message.guild?.id, `Player: ${player.guildId}`);
    ...
    if (player.queue.previous.length == 0 || previousIndex === -1)
      return new ReplyInteractionService(
        client,
        message,
        `${client.i18n.get(language, "button.player.music", "previous_notfound")}`
      );
    ...
    new ReplyInteractionService(
      client,
      message,
      `${client.i18n.get(language, "button.player.music", "previous_msg", {
        user: message.user?.id
          ? `<@${message.user.id}>`
          : `${message.user?.tag || "Người dùng không rõ"}`,
        title: previousTrack ? this.getTitle(client, previousTrack) : "Không xác định",
      })}`
    );
    return;
  }
