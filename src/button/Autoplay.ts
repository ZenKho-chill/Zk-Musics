...
export default class implements PlayerButton {
  ...
  async run(
    ...
    /////////////////////////////// Kết thúc kiểm tra Premium Role ////////////////////////////////
    ...
    if (!client.config.features.AUTOPLAY_SUPPORT) {
      const AutoplayDisabledEmbed = new EmbedBuilder()
        .setDescription(
          `${client.i18n.get(language, "client.commands.music", "autoplay_disabled", {
            user: message.user?.id
              ? `<@${message.user.id}>`
              : `${message.user?.tag || client.i18n.get(language, "server.handlers.buttons", "user_unknown")}`,
    ...
    if (player.data.get("autoplay") === true) {
      ...
      await new ReplyInteractionService(
        client,
        message,
        `${client.i18n.get(language, "client.ui", "autoplay_off", {
          mode: "Enable",
        })}`
      );
    } else {
      if (!player!.queue.current) {
        await new ReplyInteractionService(
          client,
          message,
          `${client.i18n.get(language, "client.ui", "autoplay_no_song")}`
        );
      }
      ...
      await new ReplyInteractionService(
        client,
        message,
        `${client.i18n.get(language, "client.ui", "autoplay_on", {
          mode: "Enable",
        })}`
      );
    }
