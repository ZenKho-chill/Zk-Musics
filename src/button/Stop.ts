...
export default class implements PlayerButton {
  name = "stop";
  accessableby = data.PLAYER_BUTTON.stop;
  async run(
    client: Manager,
    ...
    log.info("Stop button pressed", `User: ${message.user.id} | Guild: ${message.guild?.id} | Player: ${player.guildId}`);
    ...
    new ReplyInteractionService(
      client,
      message,
      `${client.i18n.get(language, "button.player.music", "stop_msg", {
        user: message.user?.id
          ? `<@${message.user.id}>`
          : `${message.user?.tag || "Người dùng không rõ"}`,
