...
export class ZklinkPlayerManager extends ZklinkDatabase<ZklinkPlayer> {
  ...
  async create(options: VoiceChannelOptions): Promise<ZklinkPlayer> {
    ...
    const getCustomNode = this.manager.nodes.get(
      String(options.nodeName ? options.nodeName : "")
    );
    const node = getCustomNode
      ? getCustomNode
      : await this.manager.nodes.getLeastUsed();
