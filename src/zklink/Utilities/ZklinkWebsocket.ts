...
export type ZklinkWebsocketOptions = {
  timeout?: number;
  headers?: Record<string, unknown>;
};

export type ZklinkWebsocketFHInfo = {
  opcode: number;
  fin: boolean;
...
export enum ZklinkWebsocketState {
  WAITING = "WAITING",
...
export class ZklinkWebsocket extends EventEmitter {
  protected socket: Socket | null;
  protected continueInfo: ContinueInfoType;
  protected state: ZklinkWebsocketState;

  /**
   * Phiên bản sửa đổi của lớp PWSL dành cho Zklink
   * @param url URL WebSocket cần kết nối
   * @param options Các tuỳ chọn bổ sung của PWSL
   * @instance
   */
  constructor(
    protected url: string,
    protected options: ZklinkWebsocketOptions
  ) {
    super();
    this.socket = null;
    this.continueInfo = {
    ...
    this.state = ZklinkWebsocketState.WAITING;
  ...
  /**
   * Kết nối tới đường dẫn websocket hiện tại
   * @instance
   */
  public connect() {
  ...
  protected async checkData() {
    ...
    if (data && this.state === "WAITING") {
      this.state = ZklinkWebsocketState.PROCESSING;
  ...
  /**
   * Dọn dẹp tất cả trạng thái websocket hiện tại
   * @returns boolean
   */
  cleanup() {
    if (this.socket) {
  ...
  /**
   * Gửi dữ liệu thô (Buffer) tới server websocket
   * @returns boolean
   */
  sendData(
    data: Buffer,
  ...
  /**
   * Gửi dữ liệu dạng chuỗi (string) tới server websocket
   * @returns boolean
   */
  ...
  /**
   * Đóng kết nối tới server websocket hiện tại
   * @returns boolean
   */
