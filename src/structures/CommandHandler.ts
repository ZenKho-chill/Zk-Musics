  ...
  public async deferReply() {
    if (this.interaction) {
      ...
      const data = await (this.message?.channel as TextChannel).send(
        `**${this.client.config.emojis.GLOBAL.LOADING} ${this.client.user?.username} đang xử lý...**`
      );
      return (this.msg = data);
    }
  }

  public async SilentDeferReply() {
    if (this.interaction) {
      const data = await this.interaction.deferReply({ ephemeral: false });
      return (this.msg = data);
    } else {
      const data = await (this.message?.channel as TextChannel).send({
        ...
        content: `**${this.client.config.emojis.GLOBAL.LOADING} ${this.client.user?.username} đang xử lý...**`,
        embeds: [],
      });
      return (this.msg = data);
    }
