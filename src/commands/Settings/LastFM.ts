...
export default class implements Command {
  ...
  public async execute(client: Manager, handler: CommandHandler) {
    ...
    if (type === "login") {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`${client.i18n.get("vi", "client.commands.settings", "lastfm.login_title")}`)
        .setDescription(
          `${client.i18n.get(handler.language, "client.commands.settings", "lastfm.login_desc", {
            url: authUrl,
          })}`
        )
        .setFooter({
          text: `${client.i18n.get(handler.language, "client.commands.settings", "lastfm.login_footer", {
            expired:
      ...
      if (!lastFmData || !lastFmData.sessionKey) {
        return handler.editReply({
          embeds: [
            new EmbedBuilder()
              .setDescription(
                `${client.i18n.get(handler.language, "client.commands.settings", "lastfm.not_login_desc", {
                  name: this.name[0],
                })}`
              )
              .setColor(client.color_main),
      ...
      const embed = new EmbedBuilder()
        .setTitle(
          `${client.i18n.get("vi", "client.commands.settings", "lastfm.logout_title")}`
        )
        .setDescription(
          `${client.i18n.get("vi", "client.commands.settings", "lastfm.logout_desc")}`
        )
        .setColor(client.color_main);
