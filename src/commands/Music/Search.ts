  ...
  public async execute(client: Manager, handler: CommandHandler) {
    ...
    if (isCollectorActive) {
      const responseEmbed = new EmbedBuilder()
        .setDescription(`${client.i18n.get(handler.language, "client.commands.music", "search.active")}`)
      ...
      if (!isAdminOrOwner) {
        return handler.editReply({
          embeds: [
            new EmbedBuilder()
              .setDescription(
                `${client.i18n.get(handler.language, "client.commands.music", "search.url_no_allowed")}`
              )
              .setColor(client.color_main),
          ],
        });
    ...
    if (!searchResults.tracks.length) {
      return handler.editReply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${client.i18n.get(handler.language, "client.commands.music", "search.no_result", {
    ...
    const row1 = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
      new StringSelectMenuBuilder()
        ...
        .setPlaceholder(client.i18n.get(handler.language, "client.commands.music", "search.placeholder"))
        .setMinValues(1)
        .setMaxValues(1)
        .addOptions(options)
    );
    ...
    const EmbedHome = new EmbedBuilder()
      ...
      .setDescription(
        `${client.i18n.get(handler.language, "client.commands.music", "search.desc", {
          username: client.user!.username,
          support: client.config.bot.SERVER_SUPPORT_URL,
        })}`
      )
      .setFooter({
        text: `${client.i18n.get(handler.language, "client.commands.music", "search.footer", {
          username: client.user!.username,
    ...
    collector.on("collect", async (interaction: StringSelectMenuInteraction) => {
      ...
      if (player) {
        ...
        const PlayTracksEmbed = new EmbedBuilder().setColor(client.color_second).setDescription(
          `${client.i18n.get(handler.language, "client.commands.music", "search.play_search_track", {
    ...
    collector.on("end", async (_, reason) => {
      ...
      if (reason === "time" && selectedSong) {
        const embeded = new EmbedBuilder()
          .setDescription(
            `${client.i18n.get(handler.language, "client.commands.music", "search.end_desc1", {
              title: selectedSong?.title
                ? (selectedSong.title.length > 15
                    ? selectedSong.title.substring(0, 15) + "..."
                    : selectedSong.title
        ...
        await handler.editReply({
          embeds: [
            new EmbedBuilder()
              ...
              .setColor(client.color_second)
              .setDescription(
                `${client.i18n.get(handler.language, "client.commands.music", "search.end_desc2", {
                  username: client.user!.username,
