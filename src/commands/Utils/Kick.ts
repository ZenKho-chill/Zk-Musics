  ...
  public async execute(client: Manager, handler: CommandHandler) {
    ...
    if (!memberToKick) {
      return handler.editReply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${client.i18n.get(handler.language, "client.commands.utils", "kick.user_not_found", {
                user: `<@${targetUser.id}>`,
              })}`
            )
    ...
    if (targetUser.id === guild.ownerId) {
      return handler.editReply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${client.i18n.get(handler.language, "client.commands.utils", "kick.user_ownerid", {
                user: `<@${targetUser.id}>`,
              })}`
            )
            .setColor(client.color_main),
    ...
    if (targetUserRolePosition >= requestUserRolePosition) {
      return handler.editReply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${client.i18n.get(handler.language, "client.commands.utils", "kick.user_higher_role")}`
            )
            .setColor(client.color_main),
        ],
      });
    ...
    if (targetUserRolePosition >= botRolePosition) {
      return handler.editReply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${client.i18n.get(handler.language, "client.commands.utils", "kick.bot_higher_role")}`
            )
            .setColor(client.color_main),
        ],
      });
    ...
    if (!memberToKick.kickable) {
      return handler.editReply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${client.i18n.get(handler.language, "client.commands.utils", "kick.error", {
                user: `<@${targetUser.id}>`,
              })}`
            )
            .setColor(client.color_main),
    ...
    try {
      await targetUser.send({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${client.i18n.get(handler.language, "client.commands.utils", "kick.success_dm", {
    ...
    handler.editReply({
      embeds: [
        new EmbedBuilder()
          .setDescription(
            `${client.i18n.get(handler.language, "client.commands.utils", "kick.success", {
              user: `<@${targetUser.id}>`,
              reason: reason || "Không có lý do được cung cấp",
              moderator: `<@${handler.user!.id}>`,
            })}`
