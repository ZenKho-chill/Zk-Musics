...
export class WebServer {
      ...
      try {
        const response = await axios.get("http://ws.audioscrobbler.com/2.0/", {
          params: {
            ...
            api_key:
              this.client.config.features.WebServer.LAST_FM_SCROBBLED.ApiKey,
            token: token,
        ...
        const redirectUrl = `${
          this.client.config.features.WebServer.LAST_FM_SCROBBLED
            .RedirectOnSuccess
        }?user=${encodeURIComponent(userId)}&token=${encodeURIComponent(
          token
        )}`;
  ...
  generateApiSignature(params: { [key: string]: string }) {
    ...
    const stringToHash =
      sortedParams +
      this.client.config.features.WebServer.LAST_FM_SCROBBLED.Secret;
  ...
  setupVoteWebhook() {
    ...
    const webhook = new Webhook(
      client.config.features.WebServer.TOPGG_VOTELOGS.TopGgAuth
    );
    ...
    this.app.post(
      ...
      webhook.listener(async (vote) => {
        ...
        const channelId =
          client.config.features.WebServer.TOPGG_VOTELOGS.LogVoteChannelID;
        ...
        try {
          ...
          client.logger.info(
            WebServer.name,
            `${voteUser.tag} vừa vote trên Top.gg`
          );
        } catch (error) {
    ...
    this.client.on(
      ...
      async (interaction: Interaction): Promise<void> => {
        ...
        if (interaction.customId === "reminder_vote") {
          ...
          const existingReminder = await this.client.db.VoteReminders.get(
            reminderUserId
          );
  ...
  expose() {
    ...
    this.client.logger.info(
      WebServer.name,
      `WebServer đang chạy ở cổng: ${this.port}`
    );
  }
}
